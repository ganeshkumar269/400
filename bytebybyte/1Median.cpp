...
float solve(v arr1,v arr2,int l1,int r1,int l2,int r2){
    int n = r1 - l1 + 1;
    int m = r2 - l2 + 1;
    float med1,med2;
    if(n > 0)
    if(n&1) med1 = arr1[(r1+l1)/2];
    else med1 = (arr1[(r1+l1)/2]+arr1[(r1+l1)/2 + 1])/2;
    if(m > 0)
    if(m&1) med2 = arr1[(r2+l2)/2];
    else med2 = (arr2[(r2+l2)/2]+arr2[(r2+l2)/2 + 1])/2;
    if(med1 < med2) return solve(arr1,arr2,(r1+l1)/2,r1,l1,(r2+l2)/2);
    if(med2 < med1) return solve(arr1,arr2,l1,(r1+l1)/2,(r2+l2)/2,r2);
    return med1;
}
...
int main()
{
    ...
    REP(i,n) cin>>arr1[i];
    REP(i,m) cin>>arr2[i];
    cout << solve(arr1,arr2,0,n-1,0,m-1) << endl;
